...
class TObject{
    ...
    public function toString(){
        var ret="#"+id+" : "+type+";\n";
        for(r in references)ret+="\t "+r.name+" = #"+r.obj.id+"\n";
        return ret.substr(0,ret.length-1);
    }
}
#if haxe3 typedef IntHash<T>=Map<Int,T>;
#end
#if nape_swc@:keep #end
class Graph{
    ...
    public function push(obj:Dynamic):PushResult{
        if(Reflect.isObject(obj)){
            ...
            else return new PushResult(false,untyped dict[obj]);
        }
        else return null;
    }
    public function toString(){
        var ret="";
        for(o in objects)ret+=o.toString()+"\n";
        return ret;
    }
